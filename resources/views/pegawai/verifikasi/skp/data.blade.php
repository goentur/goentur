<table id="datatables" class="table table-sm table-striped table-hover table-bordered dt-responsive" style="border-collapse: collapse; border-spacing: 0px; width: 100%;">
    ...
    <tbody>
        ...
        <tr>
            <td class="text-center">
                <a href="javascript:void(0)" class="btn btn-primary btn-sm waves-effect waves-light lihat-skp" data-toggle="modal" data-target="#full-width-modal" data-toggle="tooltip" data-placement="right" title="data skp" data-id="{{ encrypt($k->id_pegawai_jabatan) }}"><i class="mdi mdi-file-word-outline"></i></a>
            </td>
            <td>{{ $k->nip }}</td>
            <td>{{ $k->gelar_depan }} {{ $k->nama_pegawai }} {{ $k->gelar_belakang }}</td>
            <td>
